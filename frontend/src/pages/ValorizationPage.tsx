...
export function ValorizationPage() {
  const { data: gusCpi } = useGusCpi()
  const cpiValue = gusCpi?.value ?? 4.5
  const cpiDisplay = `${cpiValue.toFixed(1).replace('.', ',')}%`
  const cpiSub = gusCpi ? `CPI Q${gusCpi.quarter} ${gusCpi.year}` : 'CPI Q1 2026'
  const kpis = [
    ...BASE_KPIS,
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      >
        <div>
          <h1
            style={{ fontSize: 22, fontWeight: 700, color: '#1a1714', margin: 0, marginBottom: 2 }}
          >
            Waloryzacja
          </h1>
      ...
      <div style={{ ...card, overflow: 'hidden', marginBottom: 16 }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #f2f0ed' }}>
          ...
          <div style={{ fontSize: 12, color: '#9e9389', marginTop: 2 }}>
            ...
            const bc =
              v.statusType === 'urgent'
                ...
                : v.statusType === 'warning'
                  ? '#d69e2e'
                  : '#38a169'
            const currentValue = gusCpi ? cpiDisplay : v.current
            return (
              <div
                key={v.contract}
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1.8fr 1fr 1fr 1fr 1fr 100px',
                  alignItems: 'center',
                  gap: 12,
                  padding: '14px 16px',
                  borderRadius: 8,
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 10, color: '#9e9389', marginBottom: 2, fontWeight: 600 }}>
                    INDEKS / PRÓG
                  </div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#e85c04' }}>
                    {currentValue}
                  </div>
                </div>
                <div>
                ...
                <div style={{ textAlign: 'right' }}>
                  <span
                    style={{
                      ...
                      borderRadius: 20,
                      fontWeight: 600,
                      background: VAL_S[v.statusType]?.bg,
                      color: VAL_S[v.statusType]?.color,
                    }}
                  >
                    {v.status}
                  </span>
          ...
          {pipeline.map((stage, i) => (
            <div key={stage.stage} style={{ position: 'relative' }}>
              ...
              >
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color: stage.color }}>
                  {stage.value}
                </div>
              </div>
            </div>
