...
/* ─── Static mock data ───────────────────────────────────────── */
const BASE_RULES = [
  { contract: 'HRK/EMP/2024/07', client: 'Empik',      index: 'CPI GUS',      threshold: 'min. 4%', effective: '2026-06-01', last: '+5,2%', lastDate: '2025-06-01', status: 'Wymaga decyzji', statusType: 'urgent'  },
  { contract: 'HRK/TN/2025/03',  client: 'TechNova',   index: 'CPI GUS',      threshold: 'min. 3%', effective: '2026-07-01', last: '+3,4%', lastDate: '2025-07-01', status: 'W trakcie',     statusType: 'warning' },
  { contract: 'HRK/MED/2023/11', client: 'MediaMarkt', index: 'CPI + koszyk', threshold: 'min. 5%', effective: '2026-05-15', last: '+0,0%', lastDate: 'brak',       status: 'Blokada',       statusType: 'urgent'  },
  { contract: 'HRK/BIE/2024/03', client: 'Biedronka',  index: 'CPI GUS',      threshold: 'min. 3%', effective: '2026-09-01', last: '+4,1%', lastDate: '2025-09-01', status: 'Gotowa',        statusType: 'good'    },
  { contract: 'HRK/LID/2024/08', client: 'Lidl Polska', index: 'CPI GUS',     threshold: 'min. 3%', effective: '2026-09-01', last: '+3,8%', lastDate: '2025-09-01', status: 'Gotowa',        statusType: 'good'    },
]
...
export function ValorizationPage() {
  ...
  const cpiUpdated = cpi
    ? new Date(cpi.fetched_at).toLocaleString('pl-PL', { dateStyle: 'short', timeStyle: 'short' })
    : null
  ...
  const kpis = [
    ...
    { label: 'AKTUALNY WSKAŹNIK GUS',    value: cpiDisplay, sub: cpiSub,                color: '#3182ce' },
  ]

  const rules = BASE_RULES.map(r => ({
    ...r,
    current: r.index.includes('CPI') ? cpiDisplay : '—',
  }))
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            ...
            style={{ background: '#e85c04', border: 'none', borderRadius: 6, padding: '7px 16px', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
          >
            Generuj aneks
          </button>
        </div>
      ...
      <Modal isOpen={isAlertModalOpen} onClose={() => setIsAlertModalOpen(false)} title="Generuj alert waloryzacyjny">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {['Krytyczny', 'Wysoki', 'Normalny'].map(p => (
                <button key={p} style={{ flex: 1, padding: '8px', borderRadius: 6, border: '1px solid #e3e0db', background: p === 'Krytyczny' ? '#fff5f0' : 'white', color: p === 'Krytyczny' ? '#e85c04' : '#6b6b6b', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                  {p}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <textarea style={{ padding: '10px 12px', borderRadius: 6, border: '1px solid #e3e0db', fontSize: 13, outline: 'none', minHeight: 80, fontFamily: 'inherit' }} defaultValue="Przypomnienie: Termin podjęcia decyzji o waloryzacji dla klienta Empik upływa za 3 dni. Wymagany akcept stawek przed wysłaniem aneksu." />
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
            <button onClick={() => setIsAlertModalOpen(false)} style={{ flex: 1, padding: '10px', borderRadius: 6, border: '1px solid #e3e0db', background: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Anuluj</button>
            <button onClick={() => setIsAlertModalOpen(false)} style={{ flex: 1, padding: '10px', borderRadius: 6, border: 'none', background: '#e85c04', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Rozeszlij alert</button>
          </div>
        </div>
      </Modal>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <select style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid #e3e0db', fontSize: 13, outline: 'none' }}>
              ...
              <option>TechNova S.A. (HRK/TN/2025/03)</option>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <input type="date" defaultValue={new Date().toISOString().slice(0, 10)} style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid #e3e0db', fontSize: 13, outline: 'none' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ background: '#f5f2ef', padding: '12px', borderRadius: 8, border: '1px solid #e3e0db' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#e85c04', marginBottom: 4 }}>💡 Wskaźnik GUS</div>
            <div style={{ fontSize: 11, color: '#6b6b6b', lineHeight: 1.4 }}>
              Aktualny wskaźnik CPI: <strong>{cpiDisplay}</strong>
              {cpi ? ` (${cpiSub}, źródło: ${cpi.source})` : ''}
              {cpiUpdated ? ` · pobrano: ${cpiUpdated}` : ''}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
            <button onClick={() => setIsAnnexModalOpen(false)} style={{ flex: 1, padding: '10px', borderRadius: 6, border: '1px solid #e3e0db', background: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Anuluj</button>
            <button onClick={() => setIsAnnexModalOpen(false)} style={{ flex: 1, padding: '10px', borderRadius: 6, border: 'none', background: '#e85c04', color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Generuj dokument</button>
          </div>
        </div>
      </Modal>
