...
function formatCpi(value: number): string {
  return value.toLocaleString('pl-PL', { minimumFractionDigits: 1, maximumFractionDigits: 2 }) + '%'
}
...
export function ValorizationPage() {
  ...
  const { data: cpiData, isLoading: cpiLoading, isError: cpiError } = useGusCpi()

  const cpiDisplay = cpiLoading
    ? '…'
    : cpiError
      ? '—'
      : formatCpi(cpiData!.value)

  const cpiSubLabel = cpiData
    ? `CPI Q${cpiData.quarter} ${cpiData.year}`
    : 'CPI GUS'
  ...
  const kpis = [
    ...
    { label: 'AKTUALNY WSKAŹNIK GUS',    value: cpiDisplay, sub: cpiSubLabel,         color: '#3182ce' },
  ]

  // Inject real CPI value into rules table
  const rules = BASE_RULES.map((r) => ({
    ...r,
    current: r.index.includes('CPI') && cpiData ? formatCpi(cpiData.value) : r.index.includes('CPI') ? cpiDisplay : '—',
  }))
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      {/* GUS source info */}
      {cpiData && (
        <div style={{ marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#9e9389' }}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          Wskaźnik CPI pobierany automatycznie z {cpiData.source} · ostatnia aktualizacja: {new Date(cpiData.fetched_at).toLocaleString('pl-PL')}
        </div>
      )}
      {cpiError && (
        <div style={{ marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#c94f02', background: '#fff5f0', padding: '6px 10px', borderRadius: 6, border: '1px solid #fdd5b8' }}>
          ...
          Nie udało się pobrać wskaźnika GUS — wyświetlane dane mogą być nieaktualne.
        </div>
      )}
            ...
            return (
              <div key={v.contract} style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 800, color: cpiLoading ? '#9e9389' : '#e85c04' }}>
                    {v.current}
                  </div>
                </div>
                <div>
