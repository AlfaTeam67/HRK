/** Centralne mapowania i walidatory dla encji Customer. */

export const CUSTOMER_STATUS_PL: Record<string, string> = {
  active: 'Aktywny',
...
export function validateCustomerForm(form: Record<string, any>): ValidationErrors {
  ...
  if (form.employee_count !== '' && form.employee_count !== undefined) {
    if (!isPositiveInt(form.employee_count)) {
      errors.employee_count = 'Liczba pracowników musi być liczbą ≥ 0.'
    }
  }

  if (form.payment_period_days !== '' && form.payment_period_days !== undefined) {
    if (!isValidPaymentDays(form.payment_period_days)) {
